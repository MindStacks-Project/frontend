...
type ChartTooltipPayload = TooltipPayload<ChartValueType, ChartNameType>;
type ChartTooltipContentProps = TooltipContentProps<
  ChartValueType,
  ChartNameType
>;
const EMPTY_CHART_PAYLOAD: ChartTooltipPayload[] = [];
...
>(
  (
    ...
    const items =
      (payload as ChartTooltipPayload[] | undefined) ?? EMPTY_CHART_PAYLOAD;
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const inferredName =
        typeof item.name === "string" || typeof item.name === "number"
          ? String(item.name)
          : undefined;
      const key = labelKey ?? dataKey ?? inferredName ?? "value";
        ...
        <div className="grid gap-1.5">
          {items.map((item, index) => {
            ...
            const indicatorColor =
              color ?? payloadDetails?.fill ?? item.color ?? undefined;
